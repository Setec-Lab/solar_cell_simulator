...
#print(Carga.fijar_resistencia(5))
...
#open a file
file = open("SAS_IVcurve_Test.txt", "w")
file.write("This notepad outputs the calculated V,I,R and the measured V,I,R from the generator")
file.write("\nThe code makes it so that the only variable updated by hand is the resistance of the load")
file.write("\nCurrently the resistance is set by hand because of technical issues with the load")
file.write("\nTime,Counter,Vc,Ic,Rc,Vm,Im,Rm,Resistance")
...
while counter <= 10:
    ...
    if (power != 0):
        ...
        ouputI = round(ResistanceInterpolation(r, i, resistance), 2)
        ouputV = round(CurrentInverseInterpolation(v, i, ouputI),2)

        #updating V and I output values
        print("Values to output: {}V. {}A".format(ouputV, ouputI))
        Fuente.aplicar_voltaje_corriente(1, ouputV, ouputI)
    ...
    #uptdate notepad 
    writeInNotepad(time.time()-inicio, counter, voltage, current, resistance, loadVoltage, loadCurrent, loadResistance)
